...
const Elements = PromiseClass.create({
    ...
    attr(name, done){
        this.execCmd('getElementAttribute', {
            ...
            done(error, ret && ret.value && ret.value.trim());
        });
    },
    ...
    text(done){
        this.execCmd('getElementText', function(error, ret){
            //reduce all consecutive whitespacing to a single ascii space and remove from edges
            //matches what we store from ui recorder recording session
            done(error, ret && ret.value && ret.value.replace(/[\s\uFEFF\xA0]+/g, ' ').trim());
        });
    },
