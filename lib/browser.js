...
const Browser = PromiseClass.create({
        ...
        self.find(using, value, function*(error, elements){
            if(elements.length > 0){
                ...
                newJson.forEach(function(element){
                    if (element) elementIds.push(element.ELEMENT || element['element-6066-11e4-a52e-4f735466cecf']);
                });
    ...
    wait(using, value, options, done){
        ...
        // CBTU-375 fix for Safari 10 fails to send keys to active element. Will send to body instead for this browser only.
        if(self.browserName == "safari" && self.capVersion == "10"){
            if(using == "active"){
                using = "body";
            }
        }
