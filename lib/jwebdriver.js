...
const JWebDriver = PromiseClass.create({
    ...
    execCmd(cmd, pathData, data, done){
        ...
        let timeout = cmd == 'newSession' ? (3*60000) : 60000; //give initial post longer timeout
        ...
        if(cmdInfo !== undefined){
            ...
            if(data){
                ...
                // encode Unicode
                data = data.replace(/[^\0-~]/g, function(ch) {
                    return "\\u" + ("000" + ch.charCodeAt().toString(16)).slice(-4);
                });
                //old code that created bad length for headers
            ...
            //console.log('jwebdriver.js - execCmd', method, url, data, headers)
            self.request({
                method: method,
                url: url,
                headers: headers,
                ...
                if(error){
                    //console.log('jwebdriver.js - execCmd got error', error);
                    done(error);
                }
                else{
                    //console.log('jwebdriver.js - execCmd DID NOT get error', response);
